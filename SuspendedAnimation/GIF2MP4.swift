...
class GIF2MP4 {
        ...
        videoWriterInput.requestMediaDataWhenReady(on: queue) {
            ...
            while index < self.gif.frames.count {
                ...
                if let cgImage = self.gif.getFrame(at: index) {
                    ...
                    let result = self.addImage(image: NSImage(cgImage: cgImage, size:cgSize), withPresentationTime: presentationTime)
                    if result == false {
    ...
    func pixelBufferFromImage(image: NSImage, pixelBufferPool: CVPixelBufferPool, size: CGSize) -> CVPixelBuffer {
        ...
        let mycgImage = image.cgImage(forProposedRect: nil, context: nil, hints: nil)
        context!.draw(mycgImage!, in: CGRect(x:x, y:y, width:newSize.width, height:newSize.height))
